...
class FileSystem:
    def __init__(self, base_path=None, tts_install_path=None):
        ...
        if tts_install_path is not None:
            self.modpath = os.path.join(
                tts_install_path, "Tabletop Simulator_Data")
        else:
    ...
    def get_dir_by_type(self, save_type):
        st = {
            SaveType.workshop: self._workshop,
            SaveType.save: self._saves,
            SaveType.chest: self._chest
        }
        return st[save_type]
    ...
    def check_dirs(self):
        ...
        for dir in [self._saves, self._chest, self._mods, self._images, self._models, self._workshop]:
            if not os.path.isdir(dir):
                logger().warn("TTS Dir missing: {}".format(dir))
                return False
        return True
    ...
    def find_image(self, basename):
        result = None
        stripname = strip_filename(basename)
    ...
    def get_filenames_in(self, search_path):
        if not os.path.isdir(search_path):
            logger().warn("Tried to search non-existent path {}.".format(search_path))
            return []
