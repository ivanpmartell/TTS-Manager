...
class Preferences():
    def __init__(self):
        self.connection = winreg.ConnectRegistry(
            None, winreg.HKEY_CURRENT_USER)
        self.registry = winreg.CreateKeyEx(
            self.connection, "Software\\TTS Manager", 0, winreg.KEY_ALL_ACCESS)
        self.changed = False
        try:
            self._locationIsUser = "True" == winreg.QueryValueEx(
                self.registry, "locationIsUser")[0]
        except FileNotFoundError as e:
            #logger().info("locationIsUser missing: {}".format(e))
            self._locationIsUser = True
        try:
            self._TTSLocation = winreg.QueryValueEx(
                self.registry, "TTSLocation")[0]
        except FileNotFoundError as e:
            #logger().info("TTSLocation missing: {}".format(e))
            self._TTSLocation = ""
        try:
            self._defaultSaveLocation = winreg.QueryValueEx(
                self.registry, "defaultSaveLocation")[0]
        except FileNotFoundError as e:
            #logger().info("defaultSaveLocation missing: {}".format(e))
            self._defaultSaveLocation = ""
        try:
            self._firstRun = "True" == winreg.QueryValueEx(
                self.registry, "firstRun")[0]
        except FileNotFoundError as e:
            #logger().info("firstRun missing: {}".format(e))
            self._firstRun = True
    ...
    def save(self):
        # No longer first run.
        self.firstRun = 0
        # Make sure all values have been created
        winreg.SetValueEx(self.registry, "locationIsUser", 0,
                          winreg.REG_SZ, str(self.locationIsUser))
        winreg.SetValueEx(self.registry, "TTSLocation", 0,
                          winreg.REG_SZ, str(self.TTSLocation))
        winreg.SetValueEx(self.registry, "defaultSaveLocation",
                          0, winreg.REG_SZ, str(self.defaultSaveLocation))
        winreg.SetValueEx(self.registry, "firstRun", 0,
                          winreg.REG_SZ, str(self._firstRun))
...
class PreferencesDialog(simpledialog.Dialog):
    def applyLocationIsUser(self, *args):
    ...
    def body(self, master):
        ...
        ttk.Label(master, text="TTS Install location:").grid(
            row=1, columnspan=3)
        ...
        ttk.Button(master, text="Browse",
                   command=self.pickTTSDir).grid(row=2, column=2)
        ttk.Label(master, text="If you have installed via Steam, this will be something like:\n \"C:\\Program Files (x86)\\Steam\\steamapps\\common\\Tabletop Simulator\"").grid(
            row=5, columnspan=2)
        ttk.Button(master, text="Validate", command=self.validate).grid(
            row=3, columnspan=3)
    ...
    def validate(self):
        ...
        if not self.preferences.validate():
            messagebox.showwarning(
                "Missing directories", "Unable to find some directories - please check your settings.")
            return False
